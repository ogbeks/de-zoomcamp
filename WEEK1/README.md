...
### Practical session
**Step 1:** Install Docker for your environment from https://www.docker.com/products/docker-desktop/
Tips: I installed Docker on mac and make use of VsCode Editor

**Step 2:** Create the Ubuntu environment on Docker container
```
docker run -it ubuntu bash 
```
The -it implies interactive. The above command will create an image of Ubuntu available from docker and enter the bash terminal

**Step 3:** Create python3.9 environment and install a dependency (pandas)
```
docker run -it --entrypoint=bash python:3.9
```
This will create an image of python with version 3.9 on a docker container. Also, the entrypoint bash ensure the bash terminal is set up
thus, we make use of python just as we do normally on our terminal 
trying ```import pandas as pd ``` will result in an error as the package is not available. Thus, we install this using pip within the python3.9 environment 
using ```pip install pandas```. Now we can use pandas within our python3.9 docker container.

However, the pandas library will be delete once the container is shutdown or the same python3.9 environment is recreated.
Hence, we need to create a container that will have the pandas setup.

**Step 4:** Create container of python3.9 and pandas
We create a Dockerfile with the following details
```
FROM python:3.9
...
From the above Dockerfile, we notice the flow used when creating a simple container. We first created the python3.9 image and added the installation of pandas. 
Also, we create a directory folder called app and copy our python script into the folder.

Next: we build the dockerfile with the code below

```
docker build -t test:pandas .
```
The test:pandas is the name of our container
Thus, we can also run the newly created container with the below command;

```
docker run -it test:pandas
```

**Step 5:** Extras: Extending the container of python3.9 and pandas with input parameters
Here we want to run our docker container and pass a given parameter. This helps to bring flexibility to how the pipeline behaves.

The new updated pipeline.py file looks like this;
...
Now we can build the test:pandas container as before using
```
docker build -t test:pandas .
```
Also, we can run the container with a date parameter that is reflected when the pipeline.py is run

```
docker run -it test:pandas 2022-01-01
```

Also, you can test how the display is with more than one parameter.
```
docker run -it test:pandas "2022-01-01" 123 hello
```
